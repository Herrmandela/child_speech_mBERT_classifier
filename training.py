...
def trainingVanilla():      # 7
    ...
    global PATH, num_labels

    # Load BertForSequenceClassification, the pretrained BERT model with a single linear classification layer on top

    model = BertForSequenceClassification.from_pretrained(
        PATH,                           # Use the 12-layer BERT model, with an uncased vocab.
        num_labels,                 # The number of output labels--2 for binary classification
...
def linear_schedule():      # 9
#def linear_schedule(epochs):

    print()
    print("Linear scheduler Chosen ++++++ in training.py")
